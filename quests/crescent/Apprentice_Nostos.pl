	...
	if ($text=~/hail/i) {
		...
		if (length($charTargets) >= 2) {			
			if (CheckForEssenceAnchor(@items)) {
				...
				$client->plugin::NPCTell($npc,"I see that you've lost your [". quest::varlink($items[$tier]) ."]. I can absolutely [". quest::saylink("replace it for you",1) ."], but not for free. It will cost you ". plugin::commify($cost * 100) ."pp.");
			}		
		} else {
		...
		if (CheckForEssenceAnchor(@items) == 0 && $client->TakeMoneyFromPP($cost * 1000 * 100, 1)) {
